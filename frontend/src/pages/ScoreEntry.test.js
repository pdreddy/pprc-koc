...
test('scoreLineupFixtures treats submitted lineups without lockedAt as ready', () => {
  expect(isLockedLineupSubmission({ submittedAt: 20, submissionStatus: 'submitted_locked' })).toBe(true);
  const fixtures = scoreLineupFixtures(
    schedule,
    {},
    {
  ...
  expect(fixtures).toHaveLength(1);
  expect(fixtures[0]).toMatchObject({ ready: true, source: 'lockedSubmissions' });
  expect(fixtures[0].team1Names).toEqual(['RR Singles', 'RR D1 A', 'RR D1 B', 'RR D2 A', 'RR D2 B']);
});
